...
module Authorization
  class InstallGenerator < Rails::Generators::Base
    ...
    class_option :create_user, type: :boolean, default: false,
                               desc: 'Creates the defined User model with attributes given.'
    class_option :commit, type: :boolean, default: false, desc: 'Performs rake tasks such as migrate and seed.'
    class_option :user_belongs_to_role, type: :boolean, default: false,
                                        desc: 'Users have only one role, which can inherit others roles.'
    ...
    def install_decl_auth
      unless options[:user_belongs_to_role]
        habtm_table_name = name.pluralize.to_s <= 'Roles' ? "#{name.pluralize}Roles" : "Roles#{name.pluralize}"
      end
      unless options[:user_belongs_to_role]
        habtm_file_glob = name.pluralize.to_s <= 'Roles' ? 'db/migrate/*create_*_roles*' : 'db/migrate/*create_roles_*'
      end
